...
* 
*/
package com.charile.sqlextention;
...
import com.charile.sqlextention.SQLExtentionInfo.DBInfo;
import com.charile.sqlextention.SQLExtentionInfo.TableInfo;
...
public class SQLExtentionHolderV3
{
	...
	public void config(SQLExtentionConfigProperty property, ApplicationContext context)
	{
		...
		log.info("[config]begin config the properties");
		Integer totalTableCounts = property.getTotalTableCounts();
		// wtf,whats the fucking meaning of this sentence
		// 分库的个数:代理dao的bean名称:
		...
		{
			log.error("[config] doesnt match the config counts ");
		...
		for (int i = 0; i < tables.length; i++)
		{
			...
			if (!(bean instanceof AbstractSQLExtentionpProxyBaseCRUDDao))
			{
				throw new RuntimeException("配置错误,配置的bean类型不是指定的类型,请确认是否继承了ISQLExtentionConfigBaseCRUDDao接口");
			}
			AbstractSQLExtentionpProxyBaseCRUDDao<?> proxyDao = (AbstractSQLExtentionpProxyBaseCRUDDao<?>) bean;
	...
	// {
	// Set<Entry<String, SQLExtentionInfo<? extends AbstractSQLExtentionModel>>>
	// entry = this.sqlExtentionRepository
	// .entrySet();
	// for (Entry<String, SQLExtentionInfo<? extends AbstractSQLExtentionModel>>
	// entry2 : entry)
	// {
	// SQLExtentionInfo<? extends AbstractSQLExtentionModel> info =
	// entry2.getValue();
	// String beanName = info.getProxyDaoBeanName();
	// Object bean = context.getBean(beanName);
	// if (!(bean instanceof ISQLExtentionConfigBaseCRUDDao))
	// {
	// throw new
	// RuntimeException("配置错误,配置的bean类型不是指定的类型,请确认是否继承了ISQLExtentionConfigBaseCRUDDao接口");
	// }
	// ISQLExtentionConfigBaseCRUDDao<?> proxyDao =
	// (ISQLExtentionConfigBaseCRUDDao<?>) bean;
	 ...
	 */
	public <T extends AbstractSQLExtentionModel> SQLExtentionDaoWrapper<T> getConcreteDao(String tablePrefixName,
			Number uniqueKey)
	{
		SQLExtentionInfo<?> info = this.sqlExtentionRepository.get(tablePrefixName);
		ISQLExtentionConfigBaseCRUDDao<?> proxyBaseCRUDDao = info.getProxyDao();
	...
	public <T extends AbstractSQLExtentionModel> AbstractSQLExtentionpProxyBaseCRUDDao<T> getProxyDao(String tableName)
	{
		SQLExtentionInfo<?> info = this.sqlExtentionRepository.get(tableName);
		ISQLExtentionConfigBaseCRUDDao<?> proxyDao = info.getProxyDao();
		return (AbstractSQLExtentionpProxyBaseCRUDDao<T>) proxyDao;
	}
	...
	public DBInfo<? extends AbstractSQLExtentionModel>[] getAllDbinfos(String tablePrefixName)
	{
		SQLExtentionInfo<?> info = this.sqlExtentionRepository.get(tablePrefixName);
		return info.getDbs();
	}

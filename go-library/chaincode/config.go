...
var (
	...
	transDesc = []base.TransBaseDescription{
	}
	argsParameter = []base.ArgsParameter{
	}
)
...
func AddConfiguration(methodName base.MethodName, methodDesc base.TransBaseDescription, argParamter base.ArgsParameter) {
...
type InMemoryBlockChainBaseConfiguration struct {
	// methods        []base.MethodName
	// argsParameters []base.ArgsParameter
	// transactionDescription    []base.TransBaseDescription
	//
	// keyConstants  []base.ObjectType
	// keyGenerators []base.KeyGenerater

	ArgsDecrypter       base.ArgsDecrypter
	ArgsCheckMap        map[base.MethodName]*base.ArgsParameter
...
func NewInMemoryBlockChainBaseConfiguration() *InMemoryBlockChainBaseConfiguration {
	...
	c.BaseConfigServiceImpl = base.NewBaseConfigServiceImpl()
...
func (c *InMemoryBlockChainBaseConfiguration) Config() error2.IBaseError {
	...
	if l1 != l2 || l2 != l3 || l1 != l3 {
		return error3.NewConfigError(nil, "method和参数转换以及config描述长度必须一致")
	}

	c.ArgsCheckMap = make(map[base.MethodName]*base.ArgsParameter)
	...
	if l4 != l5 {
		return error3.NewConfigError(nil, "keyConstants和keyGenerators必须一致")
	}
	...
	if c.MultiAESSecrets==nil {
		return error3.NewConfigError(nil, "多版本密钥不可为空")
	}
...
func (b *InMemoryBlockChainBaseConfiguration) CheckAndConvt(method base.MethodName, args []string) (base.BaseFabricAfterValidModel, error2.IBaseError) {
	var (
	...
	if p, exist := b.ArgsCheckMap[method]; !exist {
		return result, error3.NewConfigError(nil, "配置错误")
	} else {
		if err := p.ArgsChecker(args); nil != err {
			return result, error3.NewArguError(err, "参数checker无法通过")
		}
		if res, baseError := p.ArgsConverter(args); nil != baseError {
			return result, error3.NewArguError(baseError, "参数转换无法通过")
		} else {
			// 判断参数是否实现了某个接口
			switch res.(type) {
			case services.IValidater:
				if e := res.(services.IValidater).Validate(); nil != e {
					return result, error3.NewArguError(e, "参数校验错误")
				}
			default:
			...
			case base.ICrypter:
				//  0 为参数 1 为version
				d, e := res.(base.ICrypter).Decrypt(args[1])
				if nil != e {
					return result, error3.NewArguError(e, "参数转换失败")
				}
				result.Req = d
			default:
				result.Req = res
			...
			if configType, exist := b.LogicDescriptionMap[method]; !exist {
				return result, error3.NewConfigError(nil, "配置错误,configType未配置")
			} else {
			...
			if nil != err {
				return result, error3.NewArguError(nil, "参数错误,版本号错误:"+args[1])
			}
...
func (b *InMemoryBlockChainBaseConfiguration) Encrypt(valueBytes []byte, version uint64) ([]byte, error2.IBaseError) {
	...
	if key, exist := b.MultiAESSecrets[version]; !exist {
		return nil, error3.NewConfigError(nil, "配置错误,无法找到匹配的密钥")
	} else {
		bytes, e := encrypt.AesEncrypt(valueBytes, []byte(key))
		if nil != e {
			return nil, error3.NewArguError(e, "加密错误")
		}
		return bytes, nil
	}
}
func (b *InMemoryBlockChainBaseConfiguration) Decrypt(encData []byte, version uint64) ([]byte, error2.IBaseError) {
	if key, exist := b.MultiAESSecrets[version]; !exist {
		...
		if nil != e {
			return nil, error3.NewArguError(e, "解密错误")
		}
		return bytes, nil
	}
}
...
func (c *InMemoryBlockChainBaseConfiguration) GetKey(stub shim.ChaincodeStubInterface, key base.ObjectType, req ...interface{}) (string, error2.IBaseError) {
	if g, exist := c.BlockChainKeyContainer[key]; !exist {
		return "", error3.NewConfigError(nil, fmt.Sprintf("key=[%v]的生成key函数不存在", key))
	} else {
...
// 		if e := Config(); nil != e {
// 			return nil, error3.NewConfigError(e, "初始化config错误")
// 		}
// 	}
...
// 	if nil == InMemoryBlockChainBaseConfiguration {
// 		if e := Config(); nil != e {
// 			return "", error3.NewConfigError(e, "初始化config错误")
// 		}
// 	}
...
// 	if nil == InMemoryBlockChainBaseConfiguration {
// 		if e := Config(); nil != e {
// 			return base.BaseFabricAfterValidModel{}, error3.NewConfigError(e, "初始化config错误")
// 		}
// 	}

...
func (pub *PublicKey) Verify(msg []byte, sign []byte) bool {
	if !pub.verify1(msg, sign) {
		return pub.verify2(msg, sign)
	}
	return true
}

// 2中方式校验
func (pub *PublicKey) verify1(msg []byte, sign []byte) bool {
	r, s, e := decodeSignature(hex.EncodeToString(sign))
	if nil != e {
		return false
	}
...
func (pub *PublicKey) verify2(msg []byte, sign []byte) bool {
	var sm2Sign sm2Signature
...
func decodeSignature(signature string) (*big.Int, *big.Int, error) {
	...
	if err != nil {
		return nil, nil, errors.New("failed to parse big.Integer R from signature:" + err.Error())
	}
	_, err = fmt.Sscanf(signatureS, "%x", &s)
	if err != nil {
		return nil, nil, errors.New("failed to parse big.Integer S from signature:" + err.Error())
	}
	return &r, &s, nil
}

// 两种方式读取国密私钥
type IPrvReader func(der, pwd []byte) (*PrivateKey, error)

//@Decrypted
func ThreeWaysReadPrivateKey(data, pwd []byte) (*PrivateKey, error) {
	str := string(data)
	...
	ways := []IPrvReader{COMMON_READER, STANDARD_READER}

	var (
		key *PrivateKey
		e   error
	)
	for _, reader := range ways {
		key, e = reader(data, pwd)
		if nil == e {
			return key, nil
		}
	}
	return key, errors.New("找不到匹配的私钥解析,err:" + e.Error())
}

func STANDARD_READER(bytes, pwd []byte) (*PrivateKey, error) {
...
func COMMON_READER(der, pwd []byte) (*PrivateKey, error) {
	priv, err := ParsePKCS8PrivateKey(der, pwd)
	return priv, err
}

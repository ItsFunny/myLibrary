...
func (b *InMemoryBlockChainBaseConfiguration) CheckAndConvt(method base.MethodName, args []string) (base.BaseFabricAfterValidModel, error2.IBaseError) {
	...
	if p, exist := b.ArgsCheckMap[method]; !exist {
		...
		if res, vlinkError := p.ArgsConverter(args); nil != vlinkError {
			return result, error2.NewArguError(vlinkError, "参数转换无法通过")
		} else {
			// 判断参数是否实现了某个接口
			switch res.(type) {
...
// 
// 
// func Encrypt(data []byte, version uint64) ([]byte, error2.IVlinkError) {
...
// }
// 
// func Decrypt(req []byte, version uint64) ([]byte, error2.IVlinkError) {
// 	if nil == InMemoryBlockChainBaseConfiguration {
// 		if e := Config(); nil != e {
// 			return nil, error2.NewConfigError(e, "初始化config错误")
// 		}
...
// }
// 
// func GetKey(stub shim.ChaincodeStubInterface, key base.ObjectType, args ...interface{}) (string, error2.IVlinkError) {
// 	if nil == InMemoryBlockChainBaseConfiguration {
// 		if e := Config(); nil != e {
...
// }
// 
// func ValidateArguAndReturn(name base.MethodName, args []string) (base.BaseFabricAfterValidModel, error2.IVlinkError) {
